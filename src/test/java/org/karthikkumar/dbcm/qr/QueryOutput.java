...
class QueryOutput {
	...
	private static final String INFOCOLSEP = " "; // No non-printable characters.
	...
	public void executeQuery(Connection pCon, String pQuery, BufferedWriter pOutBW) {
		...
		try {
			...
			for (i = 1; i <= cols; i++) {
				...
				if (mCSizes[i - 1] < mCNames[i - 1].length())
					mCSizes[i - 1] = mCNames[i - 1].length(); // Fit length
				if (type == java.sql.Types.DATE || type == java.sql.Types.TIME || type == java.sql.Types.TIMESTAMP
						|| type == 2013 || type == 2014)
					if (mCSizes[i - 1] < DATELENGTH)
						mCSizes[i - 1] = DATELENGTH;
				if (mCSizes[i - 1] > MAXLENGTH)
					mCSizes[i - 1] = MAXLENGTH;
				...
				if (type == java.sql.Types.NULL || type == java.sql.Types.BOOLEAN || type == java.sql.Types.BIT
						|| type == java.sql.Types.BIGINT || type == java.sql.Types.DECIMAL
						|| type == java.sql.Types.DOUBLE || type == java.sql.Types.FLOAT
						|| type == java.sql.Types.INTEGER || type == java.sql.Types.NUMERIC
						|| type == java.sql.Types.REAL || type == java.sql.Types.SMALLINT
